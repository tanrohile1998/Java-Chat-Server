...
public class MyClient implements ActionListener{
	...
	public void displayGUI(){
		...
		center1.add(new JLabel("Messages from all online users",JLabel.CENTER),"North");
		...
		east.add(new JLabel("Online Users",JLabel.CENTER),"Center");
		...
		chatWindow.pack();
		chatWindow.setTitle("Chat Server Application");
		...
		chatWindow.addWindowListener(new WindowAdapter(){
			public void windowClosing(WindowEvent ev){
				if(s!=null){
					JOptionPane.showMessageDialog(chatWindow,"you just logged out ","Exit",JOptionPane.INFORMATION_MESSAGE);
					logoutSession();
				}
				System.exit(0);
			}
	...
	public void actionPerformed(ActionEvent ev){
		...
		if(temp == loginButton){
			String uname= JOptionPane.showInputDialog(chatWindow,"Enter your name please");
			if(uname!=null){
				clientChat(uname);
			}
		}
	...
	public void clientChat(String uname){
		...
		try{
			s = new Socket(InetAddress.getLocalHost(),MyServer.PORT);//MyServer.PORT
